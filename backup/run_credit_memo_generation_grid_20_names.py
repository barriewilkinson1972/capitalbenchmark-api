...
Default grid:
- 20 recognisable obligors, UK/European-heavy with a couple of US/AI controls
...
Default run size:
    20 obligors × 2 context modes × 3 policy modes × 2 prompt modes × 1 mini model = 240 memos
...
DEFAULT_CONTEXT_MODES = ["full", "minimal"]
...
def summarize_memo(memo_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    ratings = memo_context.get("ratings", {}) or {}
    ...
    return {
        ...
        "context_mode": experiment_config.get("context_mode") or requested.get("context_mode"),
        ...
        "agency_rating": ratings.get("agency_rating"),
        "base_el_on_proposed_exposure": credit_request.get("base_expected_loss_on_proposed_exposure"),
...
def main() -> int:
    ...
    parser.add_argument("--context-modes", nargs="+", default=DEFAULT_CONTEXT_MODES)
    ...
    manifest = {
        "runner_version": "credit_memo_generation_grid_v0.1",
